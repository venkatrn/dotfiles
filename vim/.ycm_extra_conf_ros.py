...
def SearchForTranslationUnitWhichIncludesPath(compileCommandsPath, path, database):
    path = os.path.abspath(path)
    path = RemoveClosingSlash(path)
    # DebugLog("IncludesPath path: " + str(path))
    # DebugLog("db:" + str(compileCommandsPath))
    jsonData = open(compileCommandsPath)
    data = json.load(jsonData)
    # DebugLog("opened json")
    found = []
    ...
    if len(found) == 0:
        ...
        compilation_info = database.GetCompilationInfoForFile(result)
...
def GetCompilationInfoForFile(filename, database):
    ...
    if IsHeaderFile(filename):
        ...
        # Search the compile commands database for the translation unit that
        # includes this header.
        dirname = os.path.dirname(filename)
        compilation_database_file = GetCompilationDatabaseFolder(filename) + \
            "/" + "compile_commands.json"
        compilation_info = SearchForTranslationUnitWhichIncludesPath(
            compilation_database_file, dirname, database)
        if compilation_info:
            return compilation_info
